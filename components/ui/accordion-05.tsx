...
function FaqItemRow({ item, isOpen, onToggle, index }: FaqItemRowProps) {
  ...
  useEffect(() => {
    ...
    if (isFirst.current) {
      ...
      if (isOpen) {
        ...
        if (title)    gsap.set(title,    { color: "#7B35F8" });
        if (num)      gsap.set(num,      { color: "#7B35F8", opacity: 1 });
        if (iconWrap) gsap.set(iconWrap, { rotation: 45 });
        if (line)     gsap.set(line,     { scaleX: 1, opacity: 1 });
      } else {
        gsap.set(content, { height: 0, opacity: 0 });
        if (title)    gsap.set(title,    { color: "#111111" });
        if (num)      gsap.set(num,      { color: "#111111", opacity: 0.35 });
    ...
    if (isOpen) {
      ...
      if (title)    gsap.to(title,    { color: "#7B35F8", duration: 0.35, ease: "power2.out" });
      if (num)      gsap.to(num,      { color: "#7B35F8", opacity: 1, duration: 0.35, ease: "power2.out" });
    ...
    /* ── CLOSE ─────────────────────────────────────── */
      ...
      if (title)    gsap.to(title,    { color: "#111111",  duration: 0.25, ease: "power2.out" });
      if (num)      gsap.to(num,      { color: "#111111", opacity: 0.35, duration: 0.25, ease: "power2.out" });
  ...
  return (
    ...
    <AccordionPrimitive.Item
      value={item.id}
      className="border-b border-[#111111]/10 last:border-b"
    >
      <AccordionPrimitive.Header className="flex">
        ...
        >
          ...
          <p
            ...
            style={{ color: "#111111", opacity: 0.35 }}
          >
          ...
          <span ref={iconWrapRef} className="shrink-0 flex items-center justify-center w-6 h-6 transition-none">
            <PlusIcon className="w-5 h-5 text-[#111111]/30" strokeWidth={1.5} />
          </span>
        </AccordionPrimitive.Trigger>
      ...
      >
        <div className="pb-7 pl-4 md:pl-10 pr-4">
          ...
          <div
            ref={lineRef}
            className="h-px w-10 bg-[#7B35F8] mb-4 origin-left"
            style={{ transform: "scaleX(0)", opacity: 0 }}
          />
          <p className="font-sans text-[clamp(0.88rem,1.3vw,1rem)] text-[#111111]/60 leading-relaxed max-w-[52rem] pr-4">
            {item.content}
          </p>
        </div>
      </div>
